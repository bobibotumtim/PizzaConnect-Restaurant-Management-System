        ...
        function displayProducts() {
            ...
            grid.innerHTML = filteredProducts.map(product => {
                ...
                if (totalInt >= 999) {
                    ...
                    stockStatus = '<span class="' + stockColor + '">♾️ Unlimited</span>';
                } else if (totalInt === 0) {
                    stockColor = 'text-red-600';
                    stockStatus = '<span class="' + stockColor + '">❌ Out of Stock</span>';
                } else if (totalInt <= 10) {
                    stockColor = 'text-orange-600';
                    stockStatus = '<span class="' + stockColor + '">⚠️ ' + totalInt + ' left</span>';
                } else {
                    stockColor = 'text-green-600';
                    stockStatus = '<span class="' + stockColor + '">✅ ' + totalInt + ' available</span>';
                }
                ...
                return '<button onclick="handleProductClick(' + product.id + ')" ' +
                    ...
                    '<div class="text-xs text-gray-500 mt-1">' +
                        product.sizes.length + ' size • ' + stockStatus +
                    '</div>' +
                '</button>';
            }).join('');
        }
        ...
        function showSizeSelectionModal() {
            ...
            sizeGrid.innerHTML = selectedProduct.sizes.map(size => {
                ...
                if (qtyInt >= 999) {
                    ...
                    qtyIcon = '♾️';
                    qtyDisplay = '<div class="text-xs ' + qtyColor + ' font-semibold mt-1">' + qtyIcon + ' Unlimited</div>';
                } else if (qtyInt === 0) {
                    qtyColor = 'text-red-600';
                    qtyIcon = '❌';
                    qtyDisplay = '<div class="text-xs ' + qtyColor + ' font-semibold mt-1">' + qtyIcon + ' Out of Stock</div>';
                } else if (qtyInt <= 5) {
                    qtyColor = 'text-orange-600';
                    qtyIcon = '⚠️';
                    qtyDisplay = '<div class="text-xs ' + qtyColor + ' font-semibold mt-1">' + qtyIcon + ' ' + qtyInt + ' left</div>';
                } else {
                    qtyColor = 'text-green-600';
                    qtyIcon = '✅';
                    qtyDisplay = '<div class="text-xs ' + qtyColor + ' font-semibold mt-1">' + qtyIcon + ' ' + qtyInt + ' available</div>';
                }
