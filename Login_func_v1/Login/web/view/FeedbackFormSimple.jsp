...
import="dao.CustomerFeedbackDAO" %> <% // Force no-cache
response.setHeader("Cache-Control", "no-cache, no-store, must-revalidate");
response.setHeader("Pragma", "no-cache"); response.setDateHeader("Expires", 0);
String orderIdParam = request.getParameter("orderId"); int orderId = 0; boolean
validOrderId = false; if (orderIdParam != null &&
!orderIdParam.trim().isEmpty()) { try { orderId =
  ...
  <body>
    <div class="container">
      ...
      <button
        ...
        onclick="window.location.href='/Login/order-history'"
      >
        ← Quay lại
      </button>
      <% } else { %>
      ...
      <div id="successMessage" class="message success" style="display: none">
        <strong>Cảm ơn bạn đã đánh giá!</strong><br />Đánh giá của bạn đã được
        gửi thành công.
      </div>
        ...
        document
          ...
          .addEventListener("submit", async function (e) {
            ...
            try {
              ...
              const response = await fetch("/Login/submit-feedback", {
                method: "POST",
                body: formData,
              });
              ...
              if (result.success) {
                ...
                setTimeout(() => {
                  window.location.href = "/Login/order-history";
                }, 3000);
              } else {
