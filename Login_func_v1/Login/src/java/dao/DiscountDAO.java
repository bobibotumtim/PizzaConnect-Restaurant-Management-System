...
public class DiscountDAO extends DBContext {
     ...
     * Used by chatbot to show available promotions
     * 
    ...
    public List<Discount> getActiveDiscounts() {
        ...
        try (Connection connection = getConnection();
            ...
            LOGGER.info("Fetched " + discounts.size() + " active discounts");
     ...
     * Used by chatbot to check if a discount code is valid
     * 
     * @param code Discount code (description)
     * @return Discount object if valid, null otherwise
     */
    public Discount validateDiscountCode(String code) {
        ...
        try (Connection connection = getConnection();
            ...
            LOGGER.severe("Error validating discount code: " + e.getMessage());
        }
        
        LOGGER.info("Invalid discount code: " + code);
        return null;
    }
    
    /**
     * Get point-based discounts
     * Used by chatbot to show loyalty point redemption options
     * 
     * @return List of loyalty/point-based discounts
     */
    public List<Discount> getPointDiscounts() {
        ...
        String sql = "SELECT * FROM Discount " +
                     "WHERE DiscountType = 'Loyalty' " +
                     "AND IsActive = 1 " +
                     "AND (StartDate IS NULL OR StartDate <= CAST(GETDATE() AS DATE)) " +
                     "AND (EndDate IS NULL OR EndDate >= CAST(GETDATE() AS DATE)) " +
                     "ORDER BY MinOrderTotal ASC";
        ...
        try (Connection connection = getConnection();
            ...
            LOGGER.info("Fetched " + discounts.size() + " point-based discounts");
            
        } catch (SQLException e) {
            LOGGER.severe("Error fetching point discounts: " + e.getMessage());
        }
    ...
    /**
     * Get applicable discounts for an order amount
     * Used by chatbot to suggest discounts based on order total
     * 
     * @param orderTotal Order total amount
     * @return List of applicable discounts
     */
    public List<Discount> getApplicableDiscounts(double orderTotal) {
        autoUpdateDiscountStatus();
        List<Discount> discounts = new ArrayList<>();
        ...
        try (Connection connection = getConnection();
            ...
            LOGGER.info("Found " + discounts.size() + " applicable discounts for order total: " + orderTotal);
            
        } catch (SQLException e) {
            LOGGER.severe("Error fetching applicable discounts: " + e.getMessage());
        }
     ...
     * Used by chatbot to show savings
     * 
     * @param discount Discount object
     * @param orderTotal Order total amount
     * @return Discount amount
     */
    public double calculateDiscountAmount(Discount discount, double orderTotal) {
        if (discount == null || orderTotal < discount.getMinOrderTotal()) {
            return 0;
        }
        ...
        switch (discount.getDiscountType()) {
            case "Percentage":
                discountAmount = orderTotal * (discount.getValue() / 100.0);
            ...
            case "Loyalty":
                // For loyalty discounts, value represents points required
                // Actual discount calculation would depend on business logic
                discountAmount = discount.getValue();
                break;
