...
public class POSServlet extends HttpServlet {
        ...
        try {
            ...
            if (details != null) {
                for (int i = 0; i < details.size(); i++) {
                    ...
                    json.append("\"specialInstructions\": \"").append(escapeJson(detail.getSpecialInstructions()))
                            .append("\"");
                    json.append("}");
                }
            }
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp)
        ...
        try {
            ...
            double totalAmount = extractJsonDouble(jsonData, "total");
            System.out.println("💰 Total amount for payment: " + totalAmount);
            ...
            if (existingOrderId > 0) {
                ...
                try {
                    ...
                    if (success) {
                        ...
                        boolean paymentUpdated = updatePaymentForOrder(existingOrderId, totalAmount);
                        if (paymentUpdated) {
                            System.out.println("✅ Payment updated for Order #" + existingOrderId);
                        } else {
                            System.err.println("⚠️ Failed to update payment for Order #" + existingOrderId);
                        }

                        String response = "{\"success\": true, \"message\": \"Items added successfully!\", \"orderId\": "
                                + existingOrderId + "}";
                    ...
                    resp.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
                    resp.getWriter()
                            .write("{\"success\": false, \"message\": \"Exception: " + processEx.getMessage() + "\"}");
                }
                ...
                try {
                    ...
                    if (orderId > 0) {
                        ...
                        boolean paymentCreated = createPaymentForOrder(orderId, totalAmount);
                        if (paymentCreated) {
                            System.out.println("✅ Payment created for Order #" + orderId);
                        } else {
                            System.err.println("⚠️ Failed to create payment for Order #" + orderId);
                        }

                        String response = "{\"success\": true, \"message\": \"Order created successfully!\", \"orderId\": "
                                + orderId + "}";
                        resp.getWriter().write(response);
                        ...
                        resp.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
                        resp.getWriter()
                                .write("{\"success\": false, \"message\": \"Failed to create order - returned 0\"}");
     ...
     * CREATE PAYMENT for new order
     */
    private boolean createPaymentForOrder(int orderId, double totalAmount) {
        System.out.println("💰 CREATING PAYMENT for Order #" + orderId + ", Amount: " + totalAmount);
        ...
        try {
            ...
            payment.setPaymentStatus("Pending"); // Mặc định là Pending
            payment.setPaymentDate(new Date());
            payment.setQrCodeURL(""); // Có thể để trống hoặc generate QR code sau

            boolean success = paymentDAO.createPayment(payment);

            if (success) {
                System.out.println("✅ Payment created successfully! Payment ID: " + payment.getPaymentID());
                return true;
            } else {
                System.err.println("❌ Failed to create payment in database");
                return false;
            }

        } catch (Exception e) {
            System.err.println("❌ Exception creating payment: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
     ...
     * UPDATE PAYMENT for existing order
     */
    private boolean updatePaymentForOrder(int orderId, double newAmount) {
        System.out.println("💰 UPDATING PAYMENT for Order #" + orderId + ", New Amount: " + newAmount);
        ...
        try {
            ...
            if (existingPayment != null) {
                ...
                // Giữ nguyên status, chỉ cập nhật amount và date

                boolean success = paymentDAO.updatePayment(existingPayment);

                if (success) {
                    System.out.println("✅ Payment updated successfully! Payment ID: " + existingPayment.getPaymentID());
                    return true;
                } else {
                    System.err.println("❌ Failed to update payment in database");
                    return false;
                }
            } else {
                // Nếu không tìm thấy payment cũ, tạo payment mới
                System.out.println("⚠️ No existing payment found for Order #" + orderId + ", creating new payment...");
                return createPaymentForOrder(orderId, newAmount);
            }

        } catch (Exception e) {
            System.err.println("❌ Exception updating payment: " + e.getMessage());
            e.printStackTrace();
            return false;
        }
    }
    ...
    private int processOrderSimple(String jsonData, User user, int tableId) {
        ...
        try {
            ...
            try {
                ...
                // Use the existing createOrder method (customerID, employeeID, tableID, note,
                // orderDetails)
    ...
    private int createSimpleOrderInDB(String customerName, double total, double discount, int userId) {
        try {
            ...
            System.out.println("🔄 Calling orderDAO.createOrder(1, " + userId + ", 1, \"" + note + "\", "
                    + orderDetails.size() + " items)");
            int result = orderDAO.createOrder(1, userId, 1, note, orderDetails); // customerID=1, employeeID=userId,
                                                                                 // tableID=1
     ...
     * Format: "toppings":[{"toppingID":1,"toppingName":"Extra
     * Cheese","price":15000}]
     */
    ...
    private List<CartItemWithToppings> parseCartItemsWithToppings(String json) {
        ...
        try {
            ...
            for (int i = 0; i < itemStrings.size(); i++) {
                ...
                System.out.println("📝 Parsing item " + (i + 1) + ": "
                        + itemJson.substring(0, Math.min(100, itemJson.length())) + "...");
