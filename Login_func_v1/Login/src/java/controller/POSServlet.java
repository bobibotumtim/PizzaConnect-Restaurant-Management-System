...
*
     * Handle API request for toppings data
     * Toppings are now Products with CategoryID = 3
     */
    private void handleToppingsAPI(HttpServletRequest req, HttpServletResponse resp)
            throws ServletException, IOException {
        ...
        try {
            ...
            // Get all available toppings (now from Product + ProductSize where CategoryID = 3)
            List<Topping> toppings = toppingDAO.getAvailableToppings();
            ...
            for (int i = 0; i < toppings.size(); i++) {
                ...
                if (i > 0) json.append(",");
                json.append("{");
                json.append("\"toppingID\": ").append(topping.getToppingID()).append(","); // Actually ProductSizeID
                json.append("\"toppingName\": \"").append(escapeJson(topping.getToppingName())).append("\",");
                json.append("\"price\": ").append(topping.getPrice()).append(",");
                json.append("\"isAvailable\": ").append(topping.isAvailable());
                json.append("}");
            }
            ...
            System.out.println("✅ Toppings API called - returned " + toppings.size() + " toppings");
