        ...
        try {
            ...
            for (Discount discount : availableDiscounts) {
                if (!"Loyalty".equals(discount.getDiscountType())) {
                    // Check min order total condition - only get discount that has min order total <= order total
            ...
            try {
                ...
                if (customerId > 0) {
                    ...
                    if (earnedPoints > 0) {
                        ...
                        if (customer != null) {
                            int currentPoints = customer.getLoyaltyPoint();
                            int totalPoints = currentPoints + earnedPoints; //only plus pts because it had been subtract before
